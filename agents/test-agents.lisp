...
(aima/utilities:deftest agents
  ...
  ((run-environment (make-instance 'vacuum-world :max-steps 10)))

  ;;"You can turn off the display (with :stream nil), and just see the results:"
  ((run-environment (make-instance 'vacuum-world :stream nil)))
  ...
  ((run-environment (make-instance 'vacuum-world :stream nil :aspec '(reactive-vacuum-agent))))
  ...
  ((run-environment (make-instance 'vacuum-world :cspec '((at all (P 0.9 dirt))) :max-steps 10)))

  ;;"Finally, we can compare 2 or more agents over a number of trials:"
  ((agent-trials #'(lambda (&rest rest) (apply #'make-instance 'vacuum-world rest))
                 '(reactive-vacuum-agent random-vacuum-agent) :n 10))

  ;;"Now for the wumpus world"
  ((run-environment (make-instance 'wumpus-world :max-steps 10)))
